...
export default function ClassifiedDetailPage({ initialData }: { initialData?: ClassifiedListing | null }) {
  const params = useParams();
  const router = useRouter();
  const id = params.id as string;
  const { userId, isOwner } = useCurrentUser();
  const [listing, setListing] = useState<ClassifiedListing | null>(initialData || null);
