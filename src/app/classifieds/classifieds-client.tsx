      ...
      <section className="pt-12 sm:pt-16 pb-6">
        <div className="max-w-[var(--container)] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-baseline justify-between">
            ...
            <Link href="/classifieds/post/new" className="hidden sm:block px-4 py-2 bg-teal-700 text-white text-sm font-medium rounded-lg hover:bg-teal-800 transition-colors">
              Post ad
            </Link>
          </div>
          ...
          <div className="flex gap-10">
            ...
            <div className="lg:hidden w-full mb-4 -mt-2 flex flex-col gap-3">
              <div className="flex gap-1 overflow-x-auto scrollbar-none pb-1">
                <Link href="/classifieds"
                  className={`flex-shrink-0 px-3 py-1.5 rounded text-sm cursor-pointer transition-colors ${!activeCategory ? 'bg-teal-700 text-white' : 'text-gray-500'}`}>
                  All
                </Link>
                {CLASSIFIED_CATEGORIES.flatMap((p) => p.subcategories).map((sub) => (
                  <Link key={sub.slug} href={`/classifieds/${sub.slug}`}
                    className={`flex-shrink-0 px-3 py-1.5 rounded text-sm cursor-pointer transition-colors whitespace-nowrap ${activeCategory === sub.name ? 'bg-teal-700 text-white' : 'text-gray-500'}`}>
                    {sub.name}
                  </Link>
                ))}
              </div>
